...
export const PlanetList = ({ planets }: Props) => {
  ...
  const [optimisticPlanets, setOptimisticPlanetsNewPlanets] = useOptimistic(planets,
  ...
  const handleUpdatePlanet = async (planet: Planet) => {
    startTransition(async () => {
      ...
      try {
        setOptimisticPlanetsNewPlanets(data);
        const updatedPlanet = await updatePlanetAction(data);
        setOptimisticPlanetsNewPlanets(updatedPlanet);
      } catch (error) {
        setOptimisticPlanetsNewPlanets(planet);
      }
    });
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 animate-fadeIn">
      {optimisticPlanets.map((planet) => (
        <div key={planet.id} className="p-4 bg-gray-100 rounded shadow">
          ...
          <br />
          <button disabled={isPending} className='bg-blue-500 disabled:bg-gray-500 text-white p-2 rounded w-full' type='button' onClick={() => handleUpdatePlanet(planet)}>
            Actualizar
          </button>
        </div>
      ))}
